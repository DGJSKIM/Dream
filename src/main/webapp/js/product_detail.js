...
function checkVisible( elm, eval ) {
    ...
    if (eval == "object visible") return ((y < (viewportHeight + scrolltop)) 
    && (y > (scrolltop - elementHeight)));
...
function goUpdateProduct(){
	  ...
	  if((bool1 || $("input#product_name").val() == "" )&&
	   (bool2 || $("input#price").val() == "" )&&
	   (bool3 || $("input#discount_rate").val() == "")){// 정규식 다 만족하거나 다 빈칸으로 둔 상태
		let length = $("a.modalimage").not('a.hide').length;
		let real_length = $("a.modalimage").length;
		...
		{// 변경점 없고 추가이미지도 없으며 기존 이미지를 삭제하지도 않는다면
			...
			if($("input#price").val() == ""){
				$("input#price").val( $("input#price_price").val());	
			}
